...
if [ "$act_pwd" == "GPU-skeleton" ]; then
    ...
    _structure_change=0
    if [ !     $(date --reference=../GPU-skeleton/gpu-bENCH.inc +%s) \
           -eq $(date --reference=gpu-bENCH.inc +%s) ]; then
        #
        ...
        #
	if [       $(date --reference=../GPU-skeleton/benchmark_skeleton.json +%s) \
	       -eq $(date --reference=../GPU-skeleton/gpu-bENCH.inc +%s) ]; then
            #
